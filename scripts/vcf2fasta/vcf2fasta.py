...
def main():
    ...
    parser = argparse.ArgumentParser(prog="vcf2fasta.py",
        ...
        epilog="""
        Before running the code make sure that your reference FASTA file is indexed:
    ...
    parser.add_argument('--fasta', '-f', metavar='GENOME', type=str, required=True,
                        help='FASTA file with the reference genome.')
    parser.add_argument('--vcf', '-v', metavar='VCF', type=str, required=True,
                        help='VCF file.')
    parser.add_argument('--gff', '-g', metavar='GFF', type=str, required=False,
                        help='GFF file.')
    parser.add_argument('--bed', metavar='BED', type=str, required=False,
    ...
    parser.add_argument('--remove-gap', '-rg', action="store_true", default=False,
    ...
    if not os.path.exists(outdir):
        ...
        proceed = input(outdir + " exists. Files will be replaced. Do you want to proceed? [y|n]: ")
        if not re.match('^[Yy]', proceed):
            print('Exiting ...')
            sys.exit()
    ...
    #dict to store var counts per gene
    variant_counts_per_gene = {}
    ...
    for gene in genes:
        ...
        #store the vars count per gene
        variant_counts_per_gene[gene] = varsites
        
        
        if args.skip and varsites != 0:
            withdata += 1
            for featname in sequences.keys():
                with open(os.path.join(outdir, featname + ".fa"), "w") as out:
    ...
    # write summary to a file (vars/gene)
    summary_file = os.path.join(outdir, "variant_counts_summary.tsv")
    print(f"Writing variant counts summary to : {summary_file}")
    with open(summary_file, "w") as fout:
        fout.write("Gene\tVariant_Count\n")
        for g, count in variant_counts_per_gene.items():
            fout.write(f"{g}\t{count}\n")
